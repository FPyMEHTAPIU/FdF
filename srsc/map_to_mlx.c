...
/*   Updated: 2024/08/08 12:41:01 by msavelie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../fdf.h"

static t_point	*fill_image(mlx_image_t *img, t_map *map, int **nums)
{
	int	space;
	int	i;
	int	j;
	...
	while (i < map->lines)
	{
		...
		while (j < map->nums_in_line)
		{
			...
			//mlx_set_instance_depth(&mlx_inst[(i * map->nums_in_line) + j], nums[i][j] * 2);
